...
def draw_countries(screen, font, cities: Dict[str, Cidade]) -> None:
    for name, city in cities.items():
        ...
        level = city.get_cubosAtaque() if hasattr(city, 'get_cubosAtaque') else getattr(city, 'cubos', 0)
        if level == 0:
            color = (0, 255, 0)
        elif level <= 2:
            color = (255, 255, 0)
...
def main():
    ...
    menu_rects = [(pygame.Rect((WIDTH-btn_w)//2, start_y+i*(btn_h+spacing), btn_w, btn_h), text)
    ...
    count_rects = [(pygame.Rect((WIDTH-btn_w)//2, start_y2+i*(btn_h+spacing), btn_w, btn_h), cnt)
    ...
    profile_rects = [(pygame.Rect((WIDTH-btn_w)//2, start_y3+i*(btn_h+spacing), btn_w, btn_h), p)
    ...
    while True:
        ...
        if state == MENU:
            ...
            header = font.render(f'Jogador {current_player}, escolha seu perfil', True, WHITE)
            screen.blit(header, header.get_rect(center=(WIDTH//2, 80)))
            for rect, p in profile_rects:
                hover = rect.collidepoint(mx, my)
                color = COLOR_HIGHLIGHT if hover else COLOR_PRIMARY
            ...
            info = small_font.render(
                f'Jogadores: {player_count} | Perfil(s): {", ".join(selected_profiles)}', True, WHITE
            )
